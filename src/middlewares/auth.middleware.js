...
export const protect = async (req, res, next) => {
  try {
    ...
    const user = await UserRepository.findById(decoded.userId, "-__v");
...
export const requireSellerProfile = async (req, res, next) => {
  ...
  const seller = await SellerRepository.findByUser(req.user._id);
...
export const requireApprovedSeller = async (req, res, next) => {
  ...
  const seller = await SellerRepository.findApprovedByUser(req.user._id);
...
export const attachSellerProfile = async (req, res, next) => {
  if (req.user.role === "seller" || req.user.role === "customer") {
    req.seller = await SellerRepository.findByUser(req.user._id);
  }
  next();
};
