...
const assertPerUserUsage = async (coupon, userId) => {
  ...
  const userUses = await CouponRedemptionRepository.countDocuments({
    user: userId,
    coupon: coupon._id,
    status: "applied",
  });
...
const assertUserEligibility = async (coupon, userId) => {
  ...
  const deliveredCount = await OrderRepository.countDocuments({
    user: userId,
  });
...
export const restoreCouponOnCancel = async (order) => {
  ...
  const redemption = await CouponRedemptionRepository.findOne({
    order: order._id,
    status: "applied",
  }).populate("coupon");
