...
export const validateCategoryParent = async (parentId, categoryId = null) => {
  ...
  const parent = await CategoryRepository.findById(normalizedParent);
  if (!parent) {
  ...
  if (categoryId) {
    const childCount = await CategoryRepository.countDocuments({
      parent: categoryId,
      isActive: true,
    });
...
export const deactivateCategoryChildren = async (parentId) => {
  await CategoryRepository.updateMany({ parent: parentId }, { isActive: false });
};
...
export const resolveCategoryDepartment = async (parentId, department) => {
  ...
  if (normalizedParent) {
    const parent = await CategoryRepository.findById(normalizedParent).select("department");
    if (!parent) {
      const err = new Error("Parent category not found.");
      err.statusCode = 404;
      throw err;
